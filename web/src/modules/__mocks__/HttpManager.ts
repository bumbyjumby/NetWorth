...
export default class HttpManager{
    async get(url: string) {
        ...
        if(url.indexOf('getCurrencies') >=0 ){
            return ['CDN', 'AUD']
        }    
        
        if(url.indexOf('getValuesInCurrency') >= 0){
            return {
                entries: [{name:'Chequing', id:1, rate: 0, recordType: RecordType.ShortTermAsset, value:2000, monthlyPayment:0},
